...
def plot_vial_means_std_pub_from_nifti(
    contrast_files,
    metric_dir,
    output_file="vial_summary_T1.png",
    annotate=False,
    roi_image=None,
):
    """
...
def main():
    """
    Command-line interface for the T1 plotting script.
    
    Example usage:
        python plot_maps_ir.py contrast1.nii.gz contrast2.nii.gz \
               -m /path/to/metrics/ -o output_plot.png
    """
    parser = argparse.ArgumentParser(
        description="Plot vial mean ± std for inversion recovery with T₁ fitting and save fit metrics."
    )
    parser.add_argument(
